...
#file_name = 'ibm_torino_small_matrix_hhl_merged2.json'
#file_name = 'simulator_to_deneb_N2_matrix_hhl_merged2.json'
...
print('ave can error : ',np.average(errors), '+- ', np.std(errors))

errors = [np.sqrt(2*(1-fidelity)) for fidelity in hybrid_fidelities]
print('ave hybrid error : ',np.average(errors), '+- ', np.std(errors))

errors = [np.sqrt(2*(1-fidelity)) for fidelity in enhanced_fidelities]
print('ave enhanced error : ',np.average(errors), '+- ', np.std(errors))
