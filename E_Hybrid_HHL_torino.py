...
from QCL_QPE_torino import QCL_QPE
...
class E_Hybrid_HHL(QuantumCircuit):
    ...
    def construct_circuit(self, egn, clock, max_eigen, statevector=None, approx_degree=0):
        ...
        st = SwapTest(self.problem.qubits)
        #print(egn)
        #print(clock)
        ...
        circ = QuantumCircuit(1+qpe.num_qubits+self.problem.qubits, 2)# self.problem.qubits+1)
        ...
        circ.append(qpe, range(1, qpe.num_qubits+1))
        ...
        with circ.if_test((0,1)) as passed:
            circ.append(qpe.inverse(), range(1, qpe.num_qubits+1))
            circ.prepare_state(self.problem.ideal_x()[0].T.tolist()[0], qubits=range(-int(self.problem.qubits),0), label = '|x>')
            circ.append(st, range(-st.num_qubits,0))
            circ.measure(-1,1)
        return circ
    ...
    def estimate_x(self, clock, max_eigen=None, operator=None, statevector=None):
        ...
        transp = transpile(self.circ, backend)
        simulator_result = backend.run(transp, shots=1000).result()
